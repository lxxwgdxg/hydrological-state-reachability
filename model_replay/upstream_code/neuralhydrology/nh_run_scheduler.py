...
def _get_args() -> dict:
    ...
    # parser.add_argument('--gpu-ids', type=list, required=True)
    parser.add_argument('--runs-per-gpu', type=int, required=True)

    """
        This is the entry point of the program for DDPL, 
        by modifying the following lines to 'train' specific configuration files or 
        to execute 'test' for some specific folders
    """

    # for training
    args = vars(
        parser.parse_args(
            ["train", '--directory', './config_yml/local_one_ddpl_h_4', '--gpu-ids', '-1', '--runs-per-gpu', '6']))
    # for evaluating
    # args = vars(parser.parse_args(
    #    ["evaluate", '--directory', './all_results/******', '--gpu-ids', '-1', '--runs-per-gpu', '5']))
...
def schedule_runs(mode: str, directory: Path, gpu_ids: List[int], runs_per_gpu: int):
    ...
    elif mode in ["evaluate", "continue_training"]:
    ...
    # script_path = f'"{script_path.rstrip()}"'    ####hll
    running_processes = {}
    counter = 0
    while True:
