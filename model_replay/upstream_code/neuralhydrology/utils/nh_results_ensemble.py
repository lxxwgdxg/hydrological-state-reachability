...
def _metrics_to_dataframe(results: dict) -> pd.DataFrame:
...
def _main():
    ...
    parser.add_argument('--run-dirs', type=str, help='Directories of the runs to be averaged.')    #HLl
    ...
    args = vars(parser.parse_args(['--run-dirs', '../runs/lstm_36_one', '--output-dir', '../runs/all_results/lstm_36_one', '--metrics', 'all']))

    args['run_dirs'] = Path(args['run_dirs'])
    Path_list = list(Path(args['run_dirs']).glob('*'))    ####hll

    ensemble_results = create_results_ensemble(Path_list,
                                               # [Path(f) for f in args['run_dirs']],
                                               args['best_k'],
    ...
    try:
        df = BaseTester._metrics_to_dataframe(ensemble_results)         ####something are wrong in original coda, we put this def to here
