...
sys.path.append(str(Path(__file__).parent))  # hll
curPath = os.path.abspath(os.path.dirname(__file__))
sys.path.append(curPath)
rootPath = os.path.split(curPath)[0]
sys.path.append(rootPath)
sys.path.append(os.path.split(rootPath)[0])      # hll_ubuntu
...
def _get_args() -> dict:
    ...
    """
    This is the entry point of the program for DDPL, 
    by modifying the following lines to 'train' a specific configuration file or 
    to execute 'test' for a specific folder
    """

    # use this line of code to train
    args = vars(parser.parse_args(
        ["train", '--config-file', './config_yml/local_one_ddpl_h_4/run_config_02152100_DDPL_4_11.yml', '--gpu', '-1']))
    # use this line of code to train with multiple gpu(s)
    # args = vars(parser.parse_args(
    # ["train", '--config-file', './config_yml/global_531_ddpl_h_4/run_config_531_11_DDPL_h_4.yml', '--gpu', '0', '1']))
    # use this line of code to evaluate
    # args = vars(parser.parse_args(
    # ["evaluate", "--run-dir", "./runs/DSG_36_hll6_bigtata_s/DSG_is_h_36_bigdata_32_111_1302_233843", '--gpu', '-1']))
