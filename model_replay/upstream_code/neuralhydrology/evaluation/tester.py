...
class BaseTester(object):
    ...
    def evaluate(self,
        ...
        for basin in pbar:
            ...
            y_hat, y, loss, et, ss_flow, base_flow, snow, Tt, Smax = self._evaluate(model, loader, ds.frequencies)  # hll
            ...
            for freq in ds.frequencies:
                ...
                if y_hat[freq].ndim == 3 or (len(feature_scaler) == 1):
                    ...
                    # HLL
                    et_freq = et[freq] * feature_scaler + feature_center
                    ss_flow_freq = ss_flow[freq] * feature_scaler + feature_center
                    base_flow_freq = base_flow[freq] * feature_scaler + feature_center
                    snow_freq = snow[freq] * feature_scaler + feature_center
                    Tt_freq = Tt[freq] * feature_scaler + feature_center
                    Smax_freq = Smax[freq] * feature_scaler + feature_center
                ...
                # data = self._create_xarray(y_hat_freq, y_freq, et_freq, base_flow_freq, fast_flow_freq, Smax_freq)    ###hll
                data, data_s = self._create_xarray(y_hat_freq, y_freq, et_freq, ss_flow_freq, base_flow_freq, snow_freq,
                                                   Tt_freq, Smax_freq)  # hll
                ...
                xs = xarray.Dataset(data_vars=data_s, coords={'date': date_range, 'time_step': freq_range * Smax_freq.shape[-1]}) # hll
                results[basin][freq]['xr'] = xr
                results[basin][freq]['xs'] = xs  # hll
    ...
    def _save_results(self, results: dict, epoch: int = None):
        ...
        # result_file2 = self.run_dir / self.period / weight_file.stem / f"{self.period}_results.xlsx"
        # # hll
        # startrow = 1
        # startcol = 0
        # writer = pd.ExcelWriter(result_file2)
        # for k, v in results.items():
        #     qobs = results[k]['1D']['xr']['QObs(mm/d)_obs']
        #     qsim = results[k]['1D']['xr']['QObs(mm/d)_sim']
        #     #hll
        #     et = results[k]['1D']['xr']['et']
        #     ss_flow = results[k]['1D']['xr']['ss_flow']
        #     base_flow = results[k]['1D']['xr']['base_flow']
        #     snow = results[k]['1D']['xr']['snow']
        #     Tt = results[k]['1D']['xr']['Tt']
        #     Smax = results[k]['1D']['xs']['Smax']  ###HLL
        #
        #     k = pd.DataFrame([k])
        #     qobs = pd.DataFrame(qobs.data)
        #     qsim = pd.DataFrame(qsim.data)
        #     #hll
        #     et = pd.DataFrame(et.data)
        #     ss_flow = pd.DataFrame(ss_flow.data)
        #     base_flow = pd.DataFrame(base_flow.data)
        #     snow = pd.DataFrame(snow.data)
        #     Tt = pd.DataFrame(Tt.data)
        #     Smax = pd.DataFrame(Smax.data)
        #
        #     # print(k)
        #     k.to_excel(writer, sheet_name='sheet', startrow=0, startcol=startcol, index=False, header=False)
        #     qobs.to_excel(writer, sheet_name='sheet', startrow=startrow, startcol=startcol, index=False)
        #     qsim.to_excel(writer, sheet_name='sheet', startrow=startrow, startcol=startcol+1, index=False)
        #     #hll
        #     ss_flow.to_excel(writer, sheet_name='sheet', startrow=startrow, startcol=startcol + 2, index=False)
        #     base_flow.to_excel(writer, sheet_name='sheet', startrow=startrow, startcol=startcol + 3, index=False)
        #     et.to_excel(writer, sheet_name='sheet', startrow=startrow, startcol=startcol + 4, index=False)
        #     snow.to_excel(writer, sheet_name='sheet', startrow=startrow, startcol=startcol + 5, index=False)
        #     Tt.to_excel(writer, sheet_name='sheet', startrow=startrow, startcol=startcol + 6, index=False)
        #     Smax.to_excel(writer, sheet_name='sheet', startrow=startrow, startcol=startcol + 7, index=False)   ###hll
        #     # print(v.shape)
        #     startcol = startcol + 8 + Smax.shape[1]
        # writer.save()
        #hll
    ...
    def _evaluate(self, model: BaseModel, loader: DataLoader, frequencies: List[str], et=None):
        """Evaluate model"""
        predict_last_n = self.cfg.predict_last_n
        if isinstance(predict_last_n, int):
            predict_last_n = {frequencies[0]: predict_last_n}  # if predict_last_n is int, there's only one frequency

        preds, obs, et, ss_flow, base_flow, snow, Tt, Smax = {}, {}, {}, {}, {}, {}, {}, {}  #  hll
        losses = []
        with torch.no_grad():
            for data in loader:
                ...
                for freq in frequencies:
                    ...
                    y_hat_sub, y_sub, et_sub, ss_flow_sub, base_flow_sub, snow_sub, Tt_sub, Smax_sub = \
                        self._subset_targets(model, data, predictions, predict_last_n[freq], freq_key)   # hll

                    if freq not in preds:
                        preds[freq] = y_hat_sub.detach().cpu()
                        obs[freq] = y_sub.cpu()
                        # hll
                        et[freq] = et_sub.detach().cpu()
                        ss_flow[freq] = ss_flow_sub.detach().cpu()
                        base_flow[freq] = base_flow_sub.detach().cpu()
                        snow[freq] = snow_sub.detach().cpu()
                        Tt[freq] = Tt_sub.detach().cpu()
                        Smax[freq] = Smax_sub.detach().cpu()

                    else:
                        preds[freq] = torch.cat((preds[freq], y_hat_sub.detach().cpu()), 0)
                        obs[freq] = torch.cat((obs[freq], y_sub.detach().cpu()), 0)
                        # hll
                        et[freq] = torch.cat((et[freq], et_sub.detach().cpu()), 0)
                        ss_flow[freq] = torch.cat((ss_flow[freq], ss_flow_sub.detach().cpu()), 0)
                        base_flow[freq] = torch.cat((base_flow[freq], base_flow_sub.detach().cpu()), 0)
                        snow[freq] = torch.cat((snow[freq], snow_sub.detach().cpu()), 0)
                        Tt[freq] = torch.cat((Tt[freq], Tt_sub.detach().cpu()), 0)
                        Smax[freq] = torch.cat((Smax[freq], Smax_sub.detach().cpu()), 0)
            ...
            for freq in preds.keys():
                ...
                # hll
                et[freq] = et[freq].numpy()
                ss_flow[freq] = ss_flow[freq].numpy()
                base_flow[freq] = base_flow[freq].numpy()
                snow[freq] = snow[freq].numpy()
                Tt[freq] = Tt[freq].numpy()
                Smax[freq] = Smax[freq].numpy()
        ...
        return preds, obs, mean_loss, et, ss_flow, base_flow, snow, Tt, Smax   # hll

    def _get_predictions_and_loss(self, model: BaseModel, data: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, float]:
        predictions = model(data, self.scaler)        # hll if have
    ...
    def _create_xarray(self, y_hat: np.ndarray, y: np.ndarray, et, ss_flow, base_flow, snow, Tt, Smax):     # hll
        raise NotImplementedError
...
class RegressionTester(BaseTester):
    ...
    def _subset_targets(self, model: BaseModel, data: Dict[str, torch.Tensor], predictions: np.ndarray,
        ...
        # hll
        et_sub = predictions[f'et{freq}'][:, -predict_last_n:, :]
        ss_flow_sub = predictions[f'ss_flow{freq}'][:, -predict_last_n:, :]
        base_flow_sub = predictions[f'base_flow{freq}'][:, -predict_last_n:, :]
        snow_sub = predictions[f'snow{freq}'][:, -predict_last_n:, :]
        Tt_sub = predictions[f'Tt{freq}'][:, -predict_last_n:, :]
        Smax_sub = predictions[f'Smax{freq}'][:, -predict_last_n:, :]

        # return y_hat_sub, y_sub
        return y_hat_sub, y_sub, et_sub, ss_flow_sub, base_flow_sub, snow_sub, Tt_sub, Smax_sub   #  hll

    def _create_xarray(self, y_hat: np.ndarray, y: np.ndarray, et, ss_flow, base_flow, snow, Tt, Smax):
        data = {}
        # hll
        data_s = {}
        ...
        # hll
            data["et"] = (('date', 'time_step'), et[:, :, i])
            data["ss_flow"] = (('date', 'time_step'), ss_flow[:, :, i])
            data["base_flow"] = (('date', 'time_step'), base_flow[:, :, i])
            data["snow"] = (('date', 'time_step'), snow[:, :, i])
            data["Tt"] = (('date', 'time_step'), Tt[:, :, i])
        data_s["Smax"] = (('date', 'time_step'), Smax.squeeze(-2))  # HLL
        return data, data_s
...
class UncertaintyTester(BaseTester):
    ...
    def _subset_targets(self,
        ...
        ss_flow_sub = predictions[f'ss_flow{freq}'][:, -predict_last_n:, :]
        base_flow_sub = predictions[f'base_flow{freq}'][:, -predict_last_n:, :]
        fast_flow_sub = predictions[f'fast_flow{freq}'][:, -predict_last_n:, :]
        Smax_sub = predictions[f'Smax{freq}'][:, -predict_last_n:, :]
        # hll

        return y_hat_sub, y_sub, et_sub, ss_flow_sub, base_flow_sub, fast_flow_sub, Smax_sub
